     ...
     int IBUF[510];
{
  ...
  *MBYTE = BYTEORDER;   /* get machine byte ordering */
  ...
  if (open_files->unit != *INUNIT) { /* file is not already in list;open it*/
    ...
    if (byte != BYTEORDER) { /* byte swapping necessary */
      swap = TRUE;
    } 
    else {
      swap = FALSE;
     ...
     float *SCALE, *BAD;
{
  ...
  if (test != (float)jval && BYTEORDER && !(open_files->swap)) {
  ...
  if (test != (float)jval && (BYTEORDER == 0) && open_files->swap) {
    fval = RBUF[jval];
    if (idiff == 1) {
      RBUF[jval] = fval << 16;
    }
...
swapem64(INJ,NUM)
     ...
     long int *NUM;
{
  long int t1,t2,t3,t4,t5,t6,t7,t8;
